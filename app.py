...
# -----------------------------
# KOMPAKTE METADATEN + BUTTONS
# -----------------------------
st.markdown("---")
col_meta1, col_meta2, col_meta3, col_btn = st.columns([1, 1, 2, 1])
...
with col_left:
    ...
    for idx, (line_no, text, level) in enumerate(toc_compact):
        ...
        if st.button(short_text, key=f"toc_{idx}", help=text):
            st.session_state['cursor_line'] = line_no  # nur State setzen
...
II. Begründetheit"""
    ...
    content = st.text_area(
        ...
        height=900,  # maximaler Schreibplatz
        label_visibility="collapsed"
    )
    st.session_state.content = content

    # Cursor springen (JS-Trick)
    if 'cursor_line' in st.session_state:
        ...
        del st.session_state['cursor_line']  # danach löschen
